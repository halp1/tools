...
<!-- Accent CTA button -->
<button class="bg-accent text-bg font-mono font-medium">Submit</button>

<!-- Muted secondary label -->
<span class="text-muted text-xs uppercase tracking-widest">Modified</span>
...
```html
<header class="relative z-10 flex h-10 shrink-0 items-center border-b border-border bg-surface px-3">
  <!-- Logo -->
  <span class="shrink-0 font-heading text-lg tracking-[0.08em] text-accent">APPNAME</span>

  <!-- Center search -->
  <div class="relative mx-auto">
    <SearchIcon class="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-muted" size={13} />
    <input
      class="w-70 cursor-pointer rounded-none border border-border bg-bg py-1.25 pl-7.5 pr-2.5 font-mono text-sm text-text outline-none placeholder:text-[#333] focus:border-accent"
  ...
  <div class="ml-3 flex shrink-0 items-center gap-0.5">
    <button class="flex cursor-pointer items-center justify-center border-none bg-transparent p-1.5 text-muted transition-colors hover:bg-white/4 hover:text-text">
      <UploadIcon size={14} />
    </button>
    <!-- Divider -->
    <div class="mx-1 h-5 w-px bg-border"></div>
    <!-- Active icon (accent color) -->
    <button class="flex cursor-pointer items-center justify-center border-none bg-transparent p-1.5 text-accent transition-colors hover:bg-white/4">
      <KeyIcon size={14} />
    </button>
  </div>
</header>
...
<div class="relative z-1 flex min-h-screen items-center justify-center p-6">
  <div class="card relative w-full max-w-95 animate-[fadeUp_0.5s_ease_both] border border-border bg-surface px-10 py-12">
    ...
    <input class="mb-5 w-full appearance-none rounded-none border border-border bg-input-bg px-3.5 py-3 font-mono text-lg text-text outline-none placeholder:text-[#333] focus:border-accent" />

    <button class="mt-2 w-full cursor-pointer border-none bg-accent py-3.5 font-mono text-sm font-medium tracking-[0.12em] text-bg uppercase transition-[opacity,transform] hover:opacity-[0.88] active:scale-[0.99]">
      Log in
    </button>
  </div>
</div>
...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
  <!-- Panel -->
  <div class="relative w-80 border border-border bg-surface p-6 before:absolute before:-top-px before:-right-px before:h-8 before:w-8 before:border-t-2 before:border-r-2 before:border-accent">
    <!-- Header -->
    ...
    <!-- Input -->
    <label class="mb-1 block font-mono text-xs tracking-[0.16em] text-muted uppercase">Folder name</label>
    <input class="mb-5 w-full border border-border bg-input-bg px-3 py-2 font-mono text-base text-text outline-none placeholder:text-muted focus:border-accent" />

    <!-- Buttons -->
    <div class="flex justify-end gap-2">
      <button class="cursor-pointer border border-border bg-transparent px-4 py-1.5 font-mono text-sm tracking-[0.12em] text-muted uppercase transition-[color,border-color] hover:border-muted hover:text-text">
        Cancel
      </button>
      <button class="cursor-pointer border border-accent bg-accent px-4 py-1.5 font-mono text-sm font-medium tracking-[0.12em] text-bg uppercase transition-opacity hover:opacity-80">
        Create
      </button>
    </div>
  </div>
...
<div class="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2">
  <!-- Success toast -->
  <div class="flex min-w-72 max-w-sm items-start gap-3 border border-border bg-surface px-3.5 py-3 shadow-lg [border-left:3px_solid_var(--accent)]">
    <span class="min-w-0 flex-1 break-words font-mono text-xs leading-relaxed text-text">File uploaded successfully.</span>
    <button class="mt-0.5 shrink-0 cursor-pointer border-none bg-transparent p-0 text-muted transition-colors hover:text-text">
      <XIcon size={13} />
    </button>
  </div>
</div>
...
<style>
  .context-item:hover { color: var(--hover-color, var(--text)); }
</style>
```
...
```html
<aside class="relative flex shrink-0 flex-col overflow-hidden border-l border-border bg-surface" style="width: 280px">
  <!-- Resize handle -->
  <div class="absolute left-0 top-0 z-10 h-full w-1 cursor-col-resize bg-transparent transition-colors hover:bg-accent/50"></div>
  ...
  <div class="flex h-9 shrink-0 items-center gap-2 border-b border-border px-3">
    ...
    <button class="flex cursor-pointer items-center border-0 bg-transparent p-1.25 text-muted transition-colors hover:text-text">
      <XIcon size={13} />
    </button>
  </div>

  <!-- Scrollable body -->
  <div class="flex-1 overflow-y-auto p-3 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-track]:bg-transparent">
    <!-- Content -->
  </div>
</aside>
```
...
```html
<!-- Neutral -->
<span class="border border-border px-1.25 py-px text-xs tracking-widest text-muted uppercase">read</span>

<!-- Accent -->
<span class="border border-accent/30 px-1.25 py-px text-xs tracking-widest text-accent uppercase">24h 30m</span>
```
...
```html
<div class="flex animate-[fadeUp_0.2s_ease_both] flex-col gap-1.5 border border-border bg-bg px-3 py-2.5">
  <div class="text-sm text-text">key-name</div>
  <code class="font-mono text-xs text-muted">sk-••••••••abcd</code>
  <div class="flex flex-wrap items-center gap-1">
    <span class="border border-border px-1.25 py-px text-xs tracking-widest text-muted uppercase">read</span>
    <span class="border border-border px-1.25 py-px text-xs tracking-widest text-muted uppercase">list</span>
  </div>
  <div class="flex justify-end">
    <button class="flex cursor-pointer items-center border-0 bg-transparent p-1.25 text-muted transition-colors hover:text-[#ff6b6b]">
      <TrashIcon size={12} />
    </button>
  </div>
</div>
...
```html
<button class="mb-1 flex w-full cursor-pointer items-center justify-center gap-1.5 border border-border bg-transparent px-2.5 py-1.5 font-mono text-xs tracking-[0.08em] text-muted uppercase transition-all hover:border-accent hover:text-accent">
  <PlusIcon size={12} />
  New key
</button>
```
...
<div class="flex animate-[fadeUp_0.15s_ease_both] flex-col gap-2.5 border border-border bg-bg p-3">
  <div class="flex flex-col gap-1.5">
    ...
    <input class="w-full rounded-none border border-border bg-input-bg px-2.25 py-1.75 font-mono text-sm text-text outline-none placeholder:text-[#333] focus:border-accent" />
  </div>
  <div class="flex justify-end gap-1.5">
    <button class="cursor-pointer border border-border bg-transparent px-3 py-1.5 font-mono text-xs tracking-widest text-muted uppercase transition-all hover:border-muted hover:text-text">Cancel</button>
    <button class="cursor-pointer border-0 bg-accent px-3 py-1.5 font-mono text-xs font-medium tracking-widest text-bg uppercase transition-opacity hover:opacity-[0.88]">Create</button>
  </div>
</div>
```
...
```html
<!-- Inline form error -->
<p class="mb-5 border border-[rgba(255,80,80,0.3)] bg-[rgba(255,80,80,0.1)] px-3 py-2.5 text-sm text-[#ff8080]">
  Invalid credentials.
</p>

<!-- With icon -->
<p class="mb-4 flex items-center gap-1.5 border border-[rgba(255,80,80,0.3)] bg-[rgba(255,80,80,0.1)] px-3 py-2 text-sm text-[#ff8080]">
...
<button class="..." disabled={loading}>
  {#if loading}
    <LoaderIcon size={13} class="spin" /> Loading…
  {:else}
    Submit
  {/if}
</button>
